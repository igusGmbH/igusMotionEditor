...
class NanoJMotorControl {
	...
	static void initializeController() {
		...
		if (config.GetMotorAddress() == 1 || config.GetMotorAddress() == 3) 
			config.SetRotencInc( 142 ); // 4960 / 35
		else
			config.SetRotencInc( 133 ); // 4640 / 35
		
		// set the encoder direction
		if(config.GetMotorAddress() == 1 || config.GetMotorAddress() == 5 || config.GetMotorAddress() == 4)
			config.SetEncoderDirection(1);
		else
